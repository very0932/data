...
from sklearn.metrics import confusion_matrix, roc_curve, auc  # 添加必要的导入
...
# AlexNet模型定义（简化版）
class AlexNetGrading(nn.Module):
    def __init__(self, num_classes=5):
        super(AlexNetGrading, self).__init__()
        # 使用预训练的 AlexNet
...
# 训练函数
def train_alexnet(model, train_loader, test_loader, epochs=50, device='cuda'):
    criterion = nn.CrossEntropyLoss()
    optimizer = optim.SGD(model.parameters(), lr=1e-4, momentum=0.9, weight_decay=0.0001)  # 降低学习率和权重衰减
    scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, mode='max', factor=0.1, patience=5)  # 使用更适合的学习率调度器
...
if __name__ == "__main__":
    ...
    model = AlexNetGrading(num_classes=5)
    train_alexnet(model, train_loader, test_loader, epochs=50, device=device)
